...
public class Player extends DataModel<Map<Integer,PlayerInfo>>{
	...
	private Player(String serverID, String beginTime, String endTime, String levelLimit){
		super(String.format("SELECT userId,voucherTotal,yuanbao "
				+ "FROM condor_test_%s.player "
				+ "WHERE userId IN "
				+ "(SELECT DISTINCT log_user FROM condor_data_%s.log_logout WHERE log_time BETWEEN ? AND ?) AND LEVEL > ?", serverID, serverID), beginTime, endTime, levelLimit);
	}
	...
	public Map<Integer, PlayerInfo> packup(ResultSet rs) throws SQLException {
		...
		while(rs.next()){
			resultMap.put(rs.getInt("userId"),new PlayerInfo(rs.getInt("yuanbao"), rs.getInt("voucherTotal")));
		}
		return resultMap;
	}
	@Override
	public PackupSelfFactory<Map<Integer, PlayerInfo>> getModel(ServerNode serverNode) {
		...
		try {
			levelMinLimit = getLevelMinLimit(serverNode.getCreateTime());
	...
	private static String getLevelMinLimit(String createTime) throws ParseException{
		long yue4 = 3600 * 24 * 30 * 4 * 1000L;
		long yue2 = 3600 * 24 * 30 * 2 * 1000L;
		long yue1 = 3600 * 24 * 30 * 1 * 1000L;
		String levelMinLimit = "0";
		long openTime = System.currentTimeMillis() - TimeUtil.fromStringToLong(createTime);
		if(openTime > yue4){
			levelMinLimit = "150";
